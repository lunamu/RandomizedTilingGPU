...
inline unsigned int SpatialToHash(Point2D point, BBox gridBbox, int height, int width) { return (unsigned int)(((int)((point.y - gridBbox.ymin)*height)) * width + (point.x - gridBbox.xmin) * width); }
...
__device__ void eliminate_conflict(int query_idx, float range, int* cp_index_buffer,
	...
	int height,
	int* dev_priority,
	bool* dev_valid)//conflict points index buffer.
{
